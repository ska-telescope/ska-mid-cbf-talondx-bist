...
class DDR4_TESTER:
    ...
    def check_ver_id_reg(self):
        ...
        self.checker.check(id_reg == 0xFF110423, f"{self.ddr4_tester_name} VER_ID: 0x{id_reg:08X}")
    ...
    def configure_block_test(self, pattern=0):
    ...
    def check_block_pattern_test_status(self):
        ...
        self.checker.check(ddr4_tester_reg == True, f"{self.ddr4_tester_name} chk_start: {ddr4_tester_reg}")

        # Check Done: bits[0]
        # DDR4 Tester data checker finished status
        ddr4_tester_reg = self.ddr4_tester.read_field("chk_done") 
        self.checker.check(ddr4_tester_reg == True, f"{self.ddr4_tester_name} chk_done: {ddr4_tester_reg}")
        ...
        self.checker.check(ddr4_tester_reg == 0xFF, f"{self.ddr4_tester_name} chk_status: 0x{ddr4_tester_reg:02X}")
        ...
        self.checker.check(ddr4_tester_reg == 0x00000000, f"{self.ddr4_tester_name} err_cnt: {ddr4_tester_reg}")
        ...
        self.checker.check(ddr4_tester_reg == 0x00000000, f"{self.ddr4_tester_name} err_addr: 0x{ddr4_tester_reg:08X}")
    ...
    with tqdm.tqdm(desc=f"DDR4 random data/addr loop test", total=runtime*len(emif_list)+0.075, unit="Seconds", unit_scale=False, initial=0) as pbar:
        for ddr4_tester_name, ddr4_tester in ddr4_testers.items():
            ...
            while ((time_current - time_last) <= runtime):
                ...
                ddr4_tester.checker.check_quiet(rd_data == wr_data)
                time_current = time.time()
...
def ddr4_tester_block_pattern_rw_check(EMIFs, ddr4_testers, pattern):
    ...
    for ddr4_tester_name, ddr4_tester in ddr4_testers.items():
        start = time.time()
        max_addr = ddr4_tester.get_ddr4_max_addr()
        logging.info(f"max_addr: 0x{max_addr:08X}")
        rd_addr_total = length(max_addr * 1.5)
        with tqdm.tqdm(desc=f"{ddr4_tester.ddr4_tester_name} {ddr4_tester.ddr4_size} block test", total=rd_addr_total, unit="Bytes", unit_scale=False, initial=0) as pbar:
            ddr4_tester.configure_block_test(pattern)
            current = length(ddr4_tester.read_current_rd_addr_reg())
            while ddr4_tester.read_check_done_reg() != True:
                last = current
                current = length(ddr4_tester.read_current_rd_addr_reg())
                if (current < last):  # check for address wrapping
                    last = (current + max_addr) - last
                    pbar.update(last)
                else:
    ...
    for ddr4_tester_name, ddr4_tester in ddr4_testers.items():
        ...
        data_row = (
            ...
            + [ddr4_tester.read_check_start_reg()]
            + [ddr4_tester.read_check_done_reg()]
            + [ddr4_tester.get_test_status()]
...
if __name__ == "__main__":
    ...
    args = dict(
        ...
        testcase = "DDR4_Alternating_A_Pattern",
